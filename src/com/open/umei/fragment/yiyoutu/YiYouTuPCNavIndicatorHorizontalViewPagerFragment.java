 ...
 ***************************************************************************************************************************************************************************** 
 */
public class YiYouTuPCNavIndicatorHorizontalViewPagerFragment extends CommonIndicatorHorizontalViewPagerFragment {

	public static YiYouTuPCNavIndicatorHorizontalViewPagerFragment newInstance(String url, boolean isVisibleToUser) {
		YiYouTuPCNavIndicatorHorizontalViewPagerFragment fragment = new YiYouTuPCNavIndicatorHorizontalViewPagerFragment();
		fragment.url = url;
	...
	public UmeiNavJson call() throws Exception {
		...
		try {
			list = YiYouTuNavService.parseShowPCNav(url);
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public void onCallback(UmeiNavJson result) {
		...
		for (UmeiSubNavBean subBean : bean.getSubNavList()) {
			...
			if(subBean.getTitle().equals("首页")){
				fragment = YiYouTuPCMainNavPullGridFragment.newInstance(subBean.getHref(), true);
			}else{
				fragment = YiYouTuPCNavPullGridFragment.newInstance(subBean.getHref(), false);
			}
